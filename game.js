...
/** Touch-primary devices (phones, iPad, etc.): block pinch + double-tap zoom with CSS viewport. */
(function lockTouchViewportZoom() {
  ...
  /** Same-area double-tap (Safari zoom); skip if taps are far apart (e.g. lane changes). */
  const pinch = { t: 0, x: 0, y: 0 };
  document.addEventListener(
    "touchend",
    (e) => {
      const raw = e.target;
      ...
      if (el.closest(".gameover-panel__scroll")) return;
      const touch = e.changedTouches[0];
      if (!touch) return;
      const now = Date.now();
      const dt = now - pinch.t;
      const dx = touch.clientX - pinch.x;
      const dy = touch.clientY - pinch.y;
      const dist = Math.hypot(dx, dy);
      if (dt > 0 && dt < 380 && dist < 80) {
        e.preventDefault();
      }
      pinch.t = now;
      pinch.x = touch.clientX;
      pinch.y = touch.clientY;
    },
...
function renderLeaderboardInto(tbodyEl, highlightDateIso = null) {
  ...
  for (const e of rows) {
    ...
    tdName.textContent = String(e.name || "—").slice(0, PLAYER_NAME_MAX_LEN);
...
function recordLeaderboardScore(score, name, timeSec) {
  const ts =
    timeSec != null && Number.isFinite(timeSec)
      ...
      : null;
  const newEntry = {
    name: String(name || "Player").slice(0, PLAYER_NAME_MAX_LEN),
...
function beginSessionFromModal() {
  ...
  playerName = raw.slice(0, PLAYER_NAME_MAX_LEN);
